...
def pdos(atom, orbital, spin=None):
    ...
    df_master = pd.DataFrame(columns=["E"])
...
def pdos_plot_diagram(system, element=None, orbitals=[], spin_polarised=True, ax_title="Untitled", data_loc="./",
                      ...
                      tot_color=(0, 0, 0),
                      labela="Spin Up", labelb="Spin Down", total_dos=False, scalar=1):
    global fig
    global axs
    ...
    for orbital in orbitals:
        if spin_polarised is True:
            ...
            if len(x_total_axis) < len(val_up):
                val = len(val_up) - len(x_total_axis)
                val_up = val_up[val:]
                val_down = val_down[val:]

            a.plot(x_total_axis, scalar * val_up, label=labela, color=color, lw=0.5)
            a.plot(x_total_axis, scalar * -val_down,  label=labelb, color=color, lw=0.5)
            a.fill_between(x_total_axis, scalar * val_up, color=color, alpha=0.4)
            a.fill_between(x_total_axis, scalar * -val_down, color=color, alpha=0.4)
        else:
            val_up = pdos(element, orbital)
            a.plot(x_total_axis, val_up, label=labela, color=color, lw=0.4)
            a.fill_between(x_total_axis["E"], scalar * val_up, color=color, alpha=0.5)

    a.set_ylim(-y_total_axis_down.max(), y_total_axis_up.max())
